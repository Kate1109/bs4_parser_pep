...
def pep(session):
    ...
    main_tbody = find_tag(main_tag, 'tbody')
    sections_by_pep = main_tbody.find_all('tr')
    results = [('Статус', 'Количество')]
    statuses = []
    for tag in tqdm(sections_by_pep):
        first_column_tag = find_tag(tag, 'abbr')
        preview_status = first_column_tag.text[1:]
        href = find_tag(tag, 'a')['href']
        pep_num_link = urljoin(PEPS_DOC_URL, href)
        ...
        title = soup_dl.find(string='Status')
        card_status = title.find_parent().find_next_sibling().text
        statuses.append(card_status)
        try:
            if card_status not in EXPECTED_STATUS[preview_status]:
                msg = STATUS_PEP.format(
            ...
            logging.info(f'Несовпадающий статус в таблице {pep_num_link}')
        statuses.append(card_status)
    total = len(statuses)
